...
namespace LibraryManagementSystemProject.Controllers
{
    public class KitapController : Controller
    {
        ...
        {
            var kitaplar = from k in db.TBLKITAP select k; //Tblkitaptan tüm kitapları seç
            if (!string.IsNullOrEmpty(p))         //eğerki parametre(p) boş değilse
            {
        ...
        {
            var ktg = db.TBLKATEGORİ.Where(k => k.ID == p.TBLKATEGORİ.ID).FirstOrDefault();  //tblkategori id ile girilen parametrenin idsi eşit olan öğelerden ilk eşleşen kaydı alır veya eşleşmezse null alır,değeri ktg'ye ata
